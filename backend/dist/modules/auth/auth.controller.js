...
let AuthController = class AuthController {
    ...
    async login(dto, res, correlationId) {
        const { accessToken, refreshToken, user } = await this.auth.login(dto.email, dto.password, correlationId);
    ...
    async refresh(req) {
        ...
        if (!token) {
            throw new errors_1.UnauthorizedAppException('Missing refresh cookie');
        }
